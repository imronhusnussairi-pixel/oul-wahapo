...
class MainActivity : AppCompatActivity() {
    ...
    // Kalau aplikasi berstatus Device Owner (lihat CbtDeviceAdminReceiver & README),
    // paket ini didaftarkan lewat setLockTaskPackages() terlebih dulu sehingga
    // startLockTask() langsung mengunci TANPA dialog konfirmasi "screen pinning".
    // Kalau belum device owner, sistem tetap akan menampilkan dialog itu satu kali
    // (perilaku standar Android 9+ untuk aplikasi non-device-owner).
    private fun enterKioskMode() {
        try {
            if (CbtDeviceAdminReceiver.isDeviceOwner(this)) {
                val dpm = getSystemService(Context.DEVICE_POLICY_SERVICE) as DevicePolicyManager
                val admin = CbtDeviceAdminReceiver.getComponentName(this)
